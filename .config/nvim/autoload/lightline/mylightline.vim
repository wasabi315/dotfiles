" Filename: mytheme.vim (https://github.com/atelierbram/Base2Tone-vim/blob/master/autoload/lightline/colorscheme/mytheme.vim)
...
" condition vim-colors-mytheme uses to determine which set of colors
" to use.
let s:mytheme_termcolors = get(g:, 'mytheme_termcolors', 256)
if s:mytheme_termcolors != 256 && &t_Co >= 16
  let s:cuiindex = 0
elseif s:mytheme_termcolors == 256
  let s:cuiindex = 1
else
  let s:cuiindex = 2
endif
...
let g:lightline#colorscheme#mytheme#palette = lightline#colorscheme#flatten(s:p)
